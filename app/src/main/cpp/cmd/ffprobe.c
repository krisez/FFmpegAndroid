...
char *apply_size(char *st, char ch) {
    int len = strlen(st) + 2;
    char tmp[len];
    strcpy(tmp, st);
    tmp[len - 2] = ch;
...
void appendStr(char *s) {
    int len = strlen(ALL_CONTENT) + strlen(s) + 1;
    char tmp[len];
    strcpy(tmp, ALL_CONTENT);
    strcat(tmp, s);
    ALL_CONTENT = (char *) malloc(len * sizeof(char));
    strcpy(ALL_CONTENT, tmp);
}
